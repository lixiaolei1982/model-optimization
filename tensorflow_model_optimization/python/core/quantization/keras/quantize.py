...
def quantize_annotate_model(to_annotate):
  """Mark a model to be quantized.

  This function does not actually quantize anything. It merely wraps each layer
  in the model with `QuantizeAnnotate` to mark which layers need to be
  quantized.
  ...
  Args:
    to_annotate: tf.keras model to mark to be quantized.
...
def quantize_annotate_layer(to_annotate, quantize_config=None, **kwargs):
  """Mark a layer to be quantized.

  This function does not actually quantize anything. It merely wraps the layer
  with `QuantizeAnnotate` to note which layers need to be quantized.
  ...
  Args:
    to_annotate: tf.keras layer to mark to be quantized.
  ...
  if not isinstance(to_annotate, keras.layers.Layer) or isinstance(
    ...
    raise ValueError(
        '`to_annotate` can only be a tf.keras `layer` instance. It cannot be '
        'a model either.')
